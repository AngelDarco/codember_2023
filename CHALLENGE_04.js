...
function solution(text, k) {
  let res = "";
  if (k) {
    const arr = text.split("\n");
    for (let i = 0; i < arr.length; i++) {
      const test = arr[i].split("-")[0];
      const request = arr[i].split("-")[1];
      if (i === k - 1) res = fileChecker(test, request);
    }
  } else {
    const test = text.split("-")[0];
    const request = text.split("-")[1];
    res = fileChecker(test, request);
  }
  return res.check + " " + res.val;
  function fileChecker(test, request) {
    const res = [];
    for (let ii = 0; ii < test.length; ii++) {
      for (let jj = 0; jj < request.length; jj++) {
        if (test[ii] === request[jj]) res.push(request[jj]);
      }
    }
    return res.join("") === request.toString()
      ? { check: "Real", val: res.join("") }
      : { check: "Fake", val: res.join("") };
  }
}
const text = `3n3E65A-nE65A
U6Z1WWc0LP-U6Z1c0LP
la6bqS-la6bqS
lKyxSLIEj-lKyxSLIEj
7HIKAPFYsK-7HIAPFYs
