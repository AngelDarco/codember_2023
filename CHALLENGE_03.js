...
function solution(text, k) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const request = arr[i].split(":")[0];
    const minmax = request.split("-");

    const test = arr[i].split(":")[1];

    const min = minmax[0];
    const max = minmax[1].split(" ")[0];
    const key = minmax[1].split(" ")[1];
    for (let ii = 0; ii < test.length; ii++) {
      if (key === test[ii]) {
        count++;
      }
    }
    if (count < min || count > max) res.push(test.trim());
    count = 0;
  }

  return k ? res[k] : res[0];
}
const text = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
